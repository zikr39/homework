create database lesson8homework
use lesson8homework
...
------------taks------------
----1
select Category, count(ProductName) as ProductsAvailable from Products
group by Category
----2
select
...
----8
select 
Country, count(CustomerID) as TotalCostomers
from Customers
group by Country
----9
select 
...
----16
select ProductID, count(SaleAmount) from Sales
group by ProductID
----17
select ProductName from Products
...
----20
select 2012, 2013, 'Bektemir' as City, Bektemir as Population
from Population_Each_Year
union all
select 2012,2013, 'Chilonzor', Chilonzor
from Population_Each_Year
union all
select 2012, 2013, 'Yakkasaroy', Yakkasaroy
from Population_Each_Year
----21
select  
    p.ProductName,
...
UNPIVOT (
    Population FOR Year IN ([2018], [2019], [2020])
) AS unpvt;
